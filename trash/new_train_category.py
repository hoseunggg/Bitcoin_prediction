...
if __name__ == "__main__":
    ...
    for group_name, feature_list in FEATURE_GROUPS.items():
        ...
        for train_size in [30, 45, 60, 75, 90]:
            ...
            df_mean = run_model_from_index_spec(df, splits, col_list, model_type="mean", n_jobs=-1)
            ...
            merged = merged.merge(df_mean[['split','forecast']].rename(columns={'forecast':'forecast_mean'}), on='split')

            merged['train_size'] = train_size
            records.append(merged)

        df_out = pd.concat(records, ignore_index=True)
        out_file = os.path.join(config.RESULT_ERROR_DIR, f"predictions_by_group_{group_name}.csv")
        df_out.to_csv(out_file, index=False)
        print(f"Saved predictions for group {group_name}: {out_file}")
